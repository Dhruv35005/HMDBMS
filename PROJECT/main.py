...
def patient():
    ...
    if request.method == "POST":
        ...
        existing_booking = Patients.query.filter(
            Patients.dept == dept,
            Patients.time == time,
            Patients.date == date,
            Patients.slot == slot
        ...
        if existing_booking:
            flash("Slot is already occupied")
            return render_template('patient.html', doct=doct)

        # If slot is available, proceed with booking
        query = Patients(email=email, name=name, gender=gender, slot=slot, disease=disease,
                         time=time, date=date, dept=dept, number=number)
        db.session.add(query)
        db.session.commit()
        
        flash("Booking Confirmed", "info")
        return redirect('/bookings')  # Redirect to the bookings page after successful booking
